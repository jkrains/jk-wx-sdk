...
import com.jk.wx.sdk.commons.QwSuiteContextManager;
...
public class SuiteExchangeFilter implements ExchangeFilterFunction {

  private final QwSuiteContextManager qwSuiteContextManager;

  public SuiteExchangeFilter(QwSuiteContextManager qwSuiteContextManager) {
    this.qwSuiteContextManager = qwSuiteContextManager;
  }
  ...
  private Mono<ApplyRequestHolder> applyRequest(ClientRequest request) {
    ...
    return this.qwSuiteContextManager.getContext(wxAppContextId)
        .flatMap(context -> {

          String originalUrl = request.url().toString();
          String newUrl = appendAccessToken(originalUrl, "suite_access_token", context.getSuiteAccessToken());
